...
ES6에서 새롭게 도입된 원시값인 심벌도 래퍼 객체를 생성한다. 심벌은 일반적인 원시값과는 달리 리터럴 표기법으로 생성할 수 없고 Symbol 함수를 통해 생성해야 하므로 다른 원시값과는 차이가 있다. 이에 대해서는 후에 살펴보기로 하자.
...
즉, 전역 객체는 계층적 구조상 어떤 객체에도 속하지 않은 모든 빌트인 객체(표준 빌트인 객체와 호스트 객체)의 최상위 객체다. 전역 객체가 최상위 객체라는 것은 프로토타입 상속 관계상에서 최상위 객체라는 의미가 아니다. 프로토타입 상속 관계에서 최상위 객체는 Object이다. 전역 객체 자신은 어떤 객체의 프로퍼티도 아니며 객체의 계층적 구조상 표준 빌트인 객체와 호스트 객체를 프로퍼티를 소유한다는 것을 말한다.
...
// 선언하지 변수에 값을 할당하면 암묵적 전역. bar는 전역 변수가 아니라 전역 객체의 프로퍼티다.
bar = 2; // window.bar = 2
...
- let이나 const 키워드로 선언한 전역 변수는 전역 객체의 프로퍼티가 아니다. 즉, window.foo와 같이 접근할 수 없다. let이나 const 키워드로 선언한 전역 변수는 보이지 않는 개념적인 블록(전역 렉시컬 환경의 선언적 환경 레고드) 내에 존재하게 된다.
...
function foo() {
  ...
  eval('var x = 2;');
  console.log(x); // 2
  // eval('var x = 2;'); 이처럼 뒤에 넣으면 위의 문은 ReferenceError가 발생한다. 런타임 시에 실행되므로 변수 호이스팅이 일어나지 않는다.
}
...
eval 함수를 통해 사용자로부터 입력받은 콘텐츠(untrusted data)를 실행나느 것은 보안에 매우 취약하다. 또한 eval 함수를 통해 실행되는 코드는 자바스크립트 엔진에 의해 최적화가 수행되지 않으므로 일반적인 코드 실행에 비해 처리 속도가 느리다. 따라서 **eval 함수의 사용은 금지해야 한다.**
...
```javascript
// 완전한 URI
const uri = 'http://example.com?name=허재혁&job=programmer';

// encodeURI 함수는 완전한 URI를 전달받아 이스케이프 처리를 위해 인코딩한다.
const enc = encodeURI(uri);
console.log(enc);
// http://example.com?name=%ED%97%88%EC%9E%AC%ED%98%81&job=programmer

// decodeURI 함수는 인코딩된 완전한 URI를 전달받아 이스케이프 처리 이전으로 디코딩한다.
const dec = decoudeURI(enc);
console.log(dec);
// http://example.com?name=허재혁&job=programmer
```
